...
describe("CraftingSystem", () => {
  it("ピザ屋とパン屋の商品を同じ加工入口で作れる", () => {
    ...
    expect(craftProduct(state, "pizza", 1).state).toMatchObject({ pizzas: 1, wheatFlour: 5 });
    expect(craftProduct(state, "bread", 1).state).toMatchObject({ bread: 1, wheatFlour: 6 });
    expect(craftProduct(state, "hot-dog", 1).state).toMatchObject({ hotDogs: 1, wheatFlour: 6, sausage: 0 });
    expect(craftProduct(state, "croissant", 1).state).toMatchObject({ croissants: 1, wheatFlour: 5, butter: 0 });
  ...
  it("材料不足なら状態を変更しない", () => {
    const state = { ...createInitialGameState(0), wheatFlour: 1, sausage: 0 };
